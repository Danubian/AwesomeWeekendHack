...
namespace strange.examples.signals
{
	public class SignalsContext : MVCSContext
	{
		...
		{
			injectionBinder.Bind<ISpaceShipModel>().To<AwesomeSpaceShipModel>().ToSingleton();
			//injectionBinder.Bind<IExampleModel>().To<BasicExampleModel>().ToSingleton();
			injectionBinder.Bind<IAddShipService>().To<AddShipService>().ToSingleton();
			

			mediationBinder.Bind<AwesomeSpaceShipView>().To<SpaceShipMediator>();
			

			commandBinder.Bind<AddShipRequestSignal>().To<AddShipCommand>();
			...
			injectionBinder.Bind<ColorChangedSignal>().ToSingleton();
			injectionBinder.Bind<FulfillAddShipRequestSignal>().ToSingleton();
		}
	}
}
